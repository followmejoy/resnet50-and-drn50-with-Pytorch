...
#model_names = sorted(name for name in drn.__dict__ 
#    if name.islower() and not name.startswith("__") 
#    and callable(drn.__dict__[name])) 
...
if not os.path.exists(out_dir):
    os.mkdir(out_dir)
run = 0
current_dir= '{}/test-{}-{}'.format(out_dir,args.model,run) + time.strftime('_%Y-%m-%d-%H-%M', time.localtime(time.time()))
